...
var popMsgTop = {
	...
	showSuccessMsg : function(  _strTipInfo , _aryLink , _bolAutoClose )
	{
		...
		{
			popMsgTop.timer = window.setTimeout(function(){ popMsgTop.hide(); }, 3000);
		}
	},	
...
var popMsg = {
	...
	show: function(opt){
		...
		{
			popMsg.timer = window.setTimeout(function(){ popMsg.hide(); }, 3000);
		}
	},
...
var yhtCount = function(dom, config){
	...
	//开启计时器对象
	objInterval = setInterval(function(){count.start();}, 1000);
...
$(function(){
	...
	//教程图片
	$(".jqTuIconSmall").error(function(){ $(this).attr( 'src' , IMAGE_DOMAIN+"/tu_small.png" ); });
	$(".jqTuIconMiddle").error(function(){ $(this).attr( 'src' , IMAGE_DOMAIN+"/tu_middle.jpg" ); });
	$(".jqTuIconMiddler").error(function(){ $(this).attr( 'src' , IMAGE_DOMAIN+"/tu_middler.jpg" ); });
	$(".jqTuIconBig").error(function(){ $(this).attr( 'src' , IMAGE_DOMAIN+"/tu_big.jpg" ); });	
	//用户大图
	$(".jqUIconBig").error(function(){ replaceUserIcon( this , $(this).attr( '_att_uid') , 'B' );});
	//
	$(".jqUIconMiddle").error(function(){ 
		replaceUserIcon( this , $(this).attr( '_att_uid') , 'M' , $(this).attr( 'https'));
	});
	$(".jqUIconSmall").error(function(){ replaceUserIcon( this , $(this).attr( '_att_uid') , 'S' );});
});
