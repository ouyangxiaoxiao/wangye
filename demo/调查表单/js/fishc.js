var changeNum = function () {
    var value = $('#snrPollInterval').val();
    $('#rangNum').html(value);
};

var loveFishc = function (e) {
    // $(window).attr('location','https://fishc.taobao.com/');

    $('#tip').toggleClass('show');
    $('#tip-content').toggleClass('show');

    var timer = window.setInterval(function(){
        document.getElementById('timeBox').innerHTML = parseInt(document.getElementById('timeBox').innerHTML) - 1;
        if(parseInt(document.getElementById('timeBox').innerHTML) == 0)
        {
            window.clearInterval(timer);
            // 在新窗口
            // window.open("https://fishc.taobao.com/");
            window.location = 'https://fishc.taobao.com/';
        }
    }, 1000);
};
...
$.fn.RangeSlider = function (cfg) {
    ...
    $input.bind("input", function (e) {
        ...
        $input.css('background', 'linear-gradient(to right, #85ddc6, #2ebb96 ' + this.value + '%, #2ebb96)');
...
$('#snrPollInterval').RangeSlider({min: 0, max: 100, step: 1, callback: ''});


// 上传并显示
function upShow(file) {
    var file = document.getElementById('upFile');
    var imgPre = document.getElementById('imgShowimg');
        ...
        reader.onload = function (e) {
            ...
            image.onload=function(){
                ...
                if(width <= 600){
                    ...
                    imgPre.setAttribute("src", bold);


                    file.style.display="none";
                    label.innerText="祝贺鱼油上传成功哈"
                }
                else{
                    alert("上传图片宽度请小于600px");
                }
            };
            image.src= data;
        };
...
jQuery.fn.rater	= function(options) {
    ...
    //显示点击提示
    var clickTips= jQuery('<div class="rater-click-tips"><span>点星星就可以哈～</span></div>');
...
$(function(){
    var options	= {
        ...
        info_format	: function(value) {
            ...
            switch (value) {
                case 1 :
                    info	= '<div class="info-box">1分&nbsp;很不满意<div>小甲鱼课程太枯燥啦，太令人失望了！</div></div>';
                    break;
                case 2 :
                    info	= '<div class="info-box">2分&nbsp;不满意<div>小甲鱼讲得怎么像大学教授一样晦涩，想碎觉。</div></div>';
                    break;
                case 3 :
                    info	= '<div class="info-box">3分&nbsp;一般<div>讲的一般般啦，但比普通大学老师讲的好。</div></div>';
                    break;
                case 4 :
                    info	= '<div class="info-box">4分&nbsp;满意<div>课程短小有趣，迫不及待想看下一集。</div></div>';
                    break;
                case 5 :
                    info	= '<div class="info-box">5分&nbsp;非常满意<div>太棒啦！这么好的课程，让我爱上编程，找对老师，自学也超简单哦～</div></div>';
                    break;
                default :
                    info = value;
                    break;
